...
def mien(df):
    ...
    # Hiển thị biểu đồ
    plt.show()
def scattter_acoustic_vs_energy(df):
...
def TopTrack(df):
    ...
    plt.show()
def Mean_Popularity_Track_Over_Year(df):
...
def heat_map(df):
    ...
    numerical_columns = [
        'Danceability', 'Energy', 'Speechiness', 'Acousticness','Loudness','Time Signature','Track Duration (ms)',
...
def test(df):
    ...
    plt.tight_layout()
    plt.show()
def do_phan_bo_cua_do_pho_bien(df):
    plt.figure(figsize=(10, 6))
